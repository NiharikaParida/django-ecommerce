...
ID_OFFSET = 100000
KIDS_SOURCE = "dummyjson_kids"
KIDS_ID_OFFSET = 200000
...
def is_kids_clothing(item):
    tags = {str(tag).strip().lower() for tag in item.get("tags", [])}
    return item.get("category") == "tops" and ("girls' dresses" in tags or "girls dresses" in tags)
...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--kids-only", action="store_true", help="Import only clearly identified children's clothing as separate Kids products.")
    ...
    def handle(self, *args, **options):
        ...
        if options["kids_only"]:
            return self.import_kids_products(upstream_products, options["dry_run"])
    ...
    def import_kids_products(self, upstream_products, dry_run):
        eligible = [item for item in upstream_products if is_kids_clothing(item)]
        if dry_run:
            self.stdout.write(f"Found {len(eligible)} clearly identified Kids clothing products.")
            return

        imported = 0
        updated = 0
        skipped = 0
        category, _ = Category.objects.get_or_create(name="Kids")
        for item in eligible:
            external_id = str(item.get("id", "")).strip()
            image_urls = [str(image) for image in item.get("images", []) if str(image).startswith(("http://", "https://"))]
            if not external_id or not item.get("title") or not image_urls or not external_id.isdigit():
                skipped += 1
                continue
            brand_name = str(item.get("brand") or "Public Catalog").strip()[:100]
            brand, _ = Brand.objects.get_or_create(name=brand_name)
            ...
            old_price = (price / (Decimal("1") - discount_percentage / Decimal("100"))).quantize(Decimal("0.01")) if 0 < discount_percentage < 100 else price
            defaults = {
                "frontend_id": KIDS_ID_OFFSET + int(external_id),
                "name": str(item["title"])[:200],
                "category": category,
                "brand": brand,
                "price": price,
            ...
            _, created = Product.objects.update_or_create(source=KIDS_SOURCE, external_id=external_id, defaults=defaults)
            imported += int(created)
            updated += int(not created)

        self.stdout.write(self.style.SUCCESS(f"Kids import: {imported} new products and updated {updated}; skipped {skipped}. Existing manual and Women/Men API products were not modified."))
