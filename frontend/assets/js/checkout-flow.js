(function () {
  ...
  const items = cart.map((entry) => ({ ...products.find((product) => product.id === Number(entry.id)), quantity: Math.max(1, Number(entry.quantity) || 1) })).filter((item) => item.id);
  ...
  document.getElementById("checkoutItems").innerHTML = items.map((item) => `<div class="checkout-item"><span>${item.name}<small>${item.quantity} × ${money.format(item.price)}</small></span><b>${money.format(item.price * item.quantity)}</b></div>`).join("");
  ...
  form.addEventListener("submit", (event) => {
    ...
    const order = { id: `ORD-${Date.now().toString().slice(-8)}`, customer, items: items.map((item) => ({ id: item.id, name: item.name, quantity: item.quantity, price: item.price })), subtotal, discount, shipping, tax, total, createdAt: new Date().toISOString() };
    localStorage.setItem("fashionLastOrder", JSON.stringify(order));
    localStorage.removeItem("fashionCart"); localStorage.removeItem("fashionShipping"); localStorage.removeItem("fashionDiscountRate"); window.location.href = "order-success.html";
  });
})();
